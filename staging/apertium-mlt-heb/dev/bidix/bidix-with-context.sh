...
cat $CORPUS | apertium -d ../../ mt-he-morph | apertium-tagger -p -g ../../mt-he.prob > /tmp/mt-he.crp.tagged;
for i in `cat $INFILE | sed 's/^ *//g' | sed 's/ /:/g' | cut -f1 -d'$'  | sed 's/ /_/g'`; do 
	...
	# and put it in >> <<
	cat /tmp/mt-he.crp.tagged | grep "\/$LEMAPOS" | head -1 | sed "s/\/$LEMAPOS/<<\/$LEMAPOS/g" | sed 's/+$/+$/g' | python /home/fran/scripts/untag-corpus.py | sed 's/[A-Za-zışçğ0-9]\+<</ >>&/g' | sed 's/l- /l-/g'  | sed 's/  / /g' | fold -s; 
	# print out the candidate bidix line
	echo "" ; 
	echo '    <e c="  0"><p><l>'$LEMA''$POS'</l><r>@'$POS'</r></p></e>'; 
	echo "" ; 
	echo "================================================================================";
done
