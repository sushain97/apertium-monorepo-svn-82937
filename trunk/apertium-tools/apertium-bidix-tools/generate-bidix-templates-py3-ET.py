...
def generate_monodix_hash(tree):
	...
	for paradigm in root.iter('pardef'):#iterate recursively inside root to look for <pardef>
		...
		for tag in categories:
			needle = '.*__' + tag + '$';
			#re search inside the current_paradigm to look for tag 
		...
		for entrada in paradigm.iter('e'): 
		#iterate recursively inside paradigm element to look for <e> and assign it to entrada 
			...
			for symbol in entrada.iter('s'):
		#iterate recursively inside entrada element to look for <s>  and assign it to symbol
			...
			for entry in section.iter('e'):#look for <e> inside symbol and assign it to entry
				...
				# 	par = pars.attrib['n']
					...
					for hash in paradigms:
						...
						#entries[ox][n] = n.9588f1fbcc06f14e70ac3801be795d50
							if dep == 1:
...
def equal_entries(entry1, entry2):
	...
	if dep == 1:
		print >> sys.stderr, '--'
		print >> sys.stderr, 'entrada1: ', entrada1.decode('utf-8')
		print >> sys.stderr, 'entrada2: ', entrada2.decode('utf-8')
		print >> sys.stderr, '--'
...
def entry_exists(existing, new):
	existing = str('<doc>' + existing.encode('utf-8', 'ignore') + '</doc>');
	new = str('<doc>' + new.encode('utf-8', 'ignore') + '</doc>');
...
def generate_templates(tree, left_entries, right_entries):
	...
	for entry in root.findall('./section[@id="main"]/e'):
		...
		entrada = ET.tostring(entry).strip()
		...
		#Lemmas like Ynys Sant Finsent ac Yr Ynysoedd y Grenad&#238;ns are not replaced. Not in this script, not in the one given.
		...
		# print(template_matrix[left_hash][right_hash][bidix_hash].decode('utf-8').strip())
	...
	for left in template_matrix: 
		for right in template_matrix[left]: 
			for bidix in template_matrix[left][right]: 
				...
				templates[left][right] = template_matrix[left][right][bidix].decode('utf-8') + '<!-- ' + comment + ' -->'
	...
	for left in templates: 
		...
		for right in templates[left]: 
			...
			print('      <template>')
			print(templates[left][right].strip())
			print('      </template>')
			print('    </right>')
		print('  </left>')
		print('')
