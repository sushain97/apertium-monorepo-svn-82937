...
#	print(c, file=sys.stderr);
	...
	if c == '$': #{
		...
		outgoing = outgoing+max_split;
                sys.stderr.write(str((pos, outgoing, max_split, buf)) + '\n');
		weight = 1.0;
		for decomp in decomps: #{
                	sys.stderr.write('\t' + str((decomp)) + '\n');
			...
			if decomp.count('+') > 0: #{
				...
				for part in decomp.replace('+', ' +').split(' '): #{	
					...
					sys.stderr.write('\t' + str((pos+localpos, pos+localpos+1, part)) + '\n');
					if localpos == 0: #{
				...
				#}
				sys.stderr.write('\t' + str((pos, outgoing-1, decomp)) + '\n');
