...
public class InstallDialog extends javax.swing.JDialog {
    ...
    private void initPackages() throws IOException {
        ...
        ArrayList<String> installedPackagesFilenames = new ArrayList<String>(Arrays.asList(new File(ApertiumCaffeine.prefs.get("packagesPath", null)).list(ApertiumCaffeine.filter)));
            ...
            public void run() {
                ...
                for (int i = 0; i < packagesToInstall.length; i++)
                    if (packagesToInstall[i]) {
                        try {
                            ...
                            FileOutputStream fos = new FileOutputStream(new File(new File(ApertiumCaffeine.prefs.get("packagesPath", "")), packageToFilename.get(tableContent[i][1])));
                            ...
                            fos.close();
                            in.close();
                            ApertiumCaffeine.prefs.putLong("last_modified_" + packageToFilename.get(tableContent[i][1]), packageToURL.get(tableContent[i][1]).openConnection().getLastModified());
                        } catch (IOException ex) {
                            Logger.getLogger(InstallDialog.class.getName()).log(Level.SEVERE, null, ex);
                ...
                for (int i = 0; i < packagesToInstall.length; i++)
                    if (packagesToUninstall[i]) {
                        ...
                        if (!new File(new File(ApertiumCaffeine.prefs.get("packagesPath", "")), packageToFilename.get(tableContent[i][1])).delete()) {
                            JOptionPane.showMessageDialog(InstallDialog.this, "Unable to uninstall " + tableContent[i][1], "Error", JOptionPane.ERROR_MESSAGE);
                        }
                        ApertiumCaffeine.prefs.remove("last_modified_" + packageToFilename.get(tableContent[i][1]));
                    }
                dialog.dispose();
            }
        }.start();
