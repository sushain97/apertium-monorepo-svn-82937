...
echo "Source text: GPL license in spanish. Language pair: es-eu"

TEXT_FILE=gpl_es_4matxin.txt
NUM_LINES=`wc -l $TEXT_FILE | cut -f 1 -d" "`
TMP_TEXT_FILE=/tmp/test$$
RESULTS_FILE=resultsamdahl-matxin-ws
TMP_JMX=/tmp/testjmx$$
TMP_TIME=/tmp/time$$
...
cat scaleMT-amdahl-matxin.jmx.1 $TMP_TEXT_FILE scaleMT-amdahl-matxin.jmx.2 > $TMP_JMX
